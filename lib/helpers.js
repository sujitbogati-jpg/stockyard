export const WAREHOUSE_ZONE_CODES =;
...
// 🛠️ UPGRADED EXCEL ROW CONVERTER ENGINE WITH TEXT-TO-NUMBER FALLBACKS
export function excelRowToDbRow(r) {
  // Cleans green-triangle text strings out of numerical properties safely
  const cleanPrice = r["Unit Price in USD"] != null ? Number(String(r["Unit Price in USD"]).replace(/[^0-9.]/g, "")) : 0;
  const cleanQuantity = r["Quantity"] != null ? Number(String(r["Quantity"]).replace(/[^0-9.]/g, "")) : 0;
  
  const rawTotalValue = r["Total Stock Value in USD"];
  const cleanTotalValue = (rawTotalValue != null && rawTotalValue !== 0 && rawTotalValue !== "0")
    ? Number(String(rawTotalValue).replace(/[^0-9.]/g, ""))
    : (cleanQuantity * cleanPrice); // Generates calculation fallback automatically if total value column evaluates to zero
